...
app.config["SQLALCHEMY_DATABASE_URI"] = "postgresql://localhost/mithilshw5"
## Provided:
...
class UpdatePriorityForm(FlaskForm):
    update_task = StringField('Update item priority',validators=[Required()])
    submit = SubmitField('Update')
...
class DeleteButtonForm(FlaskForm):
    delete_task = StringField('Choose task to delete',validators=[Required()])
...
def one_list(ident):
    ...
    list_x = []
    for item in items:
        list_x.append((item.description,item.priority))
...
def update(item):
    ...
    if form.validate_on_submit():
        priority = form.update_task.data
...
def delete(lst):
    x = TodoList.query.filter_by(title=lst).first()
    db.session.delete(x)
    db.session.commit()
    flash(lst+" deleted")
    return redirect(url_for('all_lists'))
